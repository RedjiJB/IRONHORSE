...
**Not yet done**: `docs/DATABASE_SCHEMA.md`-equivalent doc for the new `did` column; zero-knowledge-proof work is scoped as separate future work, see "Open decisions" below — "zero trust" (no implicitly-trusted role/tier state, everything re-verifies a real signature) is fully implemented by this slice, but no literal zero-knowledge-proof cryptography exists yet.
...
7. **Zero-knowledge-proof work, scoped but not started (2026-08-21).** "Zero trust and zero knowledge system" was clarified as two distinct asks: zero-trust (no capability is trusted from a database column, every check re-verifies a real signature) is fully implemented as of Phase 2 slice 2 above. Literal ZK-proof cryptography is not — decided to build it as new, separate work, first target the geofence claim (proving a crew member was within a site's radius without the raw GPS coordinates ever appearing in a log or DB row). **Open, load-bearing caveat identified before implementation starts**: in the current custodial-crew, single-node architecture, the node receives raw GPS in plaintext (via WhatsApp) *and* is the verifier deciding `geofence_verified` — there is no second party to hide the coordinates from yet, so this proof has no privacy payoff today. Decided to build the primitive anyway, ahead of the actual privacy boundary (a federation peer or a client-facing report), rather than wait. Needs its own design pass before code: proving `(lat-centerLat)² + (lng-centerLng)² ≤ radius²` in zero knowledge is a small arithmetic circuit, not a plain Bulletproofs range proof — realistic path is `circom`+`snarkjs` (Groth16/PLONK) given the rest of the stack is Node/TS; BBS+ signatures are the separate, not-yet-chosen tool for any future selective-disclosure-of-VC-claims work (a different problem from geofence range proofs). Not implemented yet.
