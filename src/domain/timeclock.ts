// Ported from dcentral-fieldops's src/domain/timeclock.ts (see
// PRECEDENT-ARCHITECTURE.md §3) -- same resolveGeofenceVerified behavior,
// re-expressed for guards/sites. No site_id, no site geofence configured, or
// no lat/lng all fall through to false; there is no path to assert true
...
// approval handler once a supervisor review has happened. Takes
// geofenceVerified as an already-resolved boolean rather than lat/lng, so
// the same function serves both "verify now, at submission time" (the tool
// handler) and "re-verify against payload, at approval time" (the approval
// handler) without resolving twice or trusting a stale value.
export async function createTimeclockEntry(args: {
  ...
  const result = await pool.query(
    `INSERT INTO timeclock_entries (guard_id, event_type, site_id, geofence_verified, lat, lng)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
    [args.guardId, args.eventType, args.siteId ?? null, args.geofenceVerified, args.lat ?? null, args.lng ?? null],
  );
  return result.rows[0] as TimeclockEntry;
}

export async function listTimeclockEntries(guardId: string): Promise<TimeclockEntry[]> {
  const result = await pool.query(
    "SELECT * FROM timeclock_entries WHERE guard_id = $1 ORDER BY timestamp DESC",
    [guardId],
  );
  return result.rows as TimeclockEntry[];
}

// Registered once at server startup (see src/mcp/tools/timeclock.ts) -- the
// executor a supervisor approval actually runs. Re-resolves
...
export function registerTimeclockConfirmationExecutor(): void {
  registerConfirmationExecutor("timeclock_event", async (payload) => {
    ...
    const entry = await createTimeclockEntry({ guardId, eventType, siteId, geofenceVerified, lat, lng });
