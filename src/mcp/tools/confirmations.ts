...
export function registerConfirmationTools(server: McpServer): void {
  server.registerTool(
    "list_pending_confirmations",
    {
      ...
      description: "Lists every action awaiting supervisor review. Minimum tier: 2.",
  ...
  server.registerTool(
    "approve_pending_confirmation",
    {
      ...
      description:
        "Approves a pending action and actually executes it -- the reviewer must hold guard role 'supervisor' or 'admin' (checked against reviewerGuardId, not the calling agent's tier alone). Re-validates against current state, not state at submission time. Minimum tier: 3.",
      inputSchema: z.object({
        ...credentialArg,
        id: z.string().uuid(),
        reviewerGuardId: z.string().uuid(),
        approvalData: z.record(z.string(), z.unknown()).optional(),
      }),
    },
    async ({ credentialJwt, id, reviewerGuardId, approvalData }) => {
      try {
        await requireCapability(credentialJwt, "mcp:tool:approve_pending_confirmation", 3);
        const result = await approveConfirmation(id, reviewerGuardId, approvalData);
  ...
  server.registerTool(
    "reject_pending_confirmation",
    {
      ...
      description: "Rejects a pending action without executing it -- same reviewer-role check as approval. Minimum tier: 3.",
      inputSchema: z.object({
        ...credentialArg,
        id: z.string().uuid(),
        reviewerGuardId: z.string().uuid(),
    ...
    async ({ credentialJwt, id, reviewerGuardId, note }) => {
      try {
        await requireCapability(credentialJwt, "mcp:tool:reject_pending_confirmation", 3);
        const result = await rejectConfirmation(id, reviewerGuardId, note);
